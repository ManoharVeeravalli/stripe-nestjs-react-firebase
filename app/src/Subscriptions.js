...
function SubscribeToPlan(props) {
  ...
  const getSubscriptions = async () => {
    if (user.data) {
      const subs = await fetchFromAPI("stripe/subscriptions", {
        method: "GET",
      });
  ...
  const cancel = async (id) => {
    setLoading(true);
    await fetchFromAPI("stripe/subscriptions/" + id, { method: "PATCH" });
    alert("canceled!");
  ...
  const handleSubmit = async (event) => {
    ...
    const subscription = await fetchFromAPI("stripe/subscriptions", {
      body: {
        plan,
      ...
      <AuthCheck fallback={<SignIn />}>
        ...
        <div className="well">
          ...
          <button
            className={
              "btn " +
              (plan === "price_1IxuFlSJzInEwQYN8hPRSSLF"
                ? "btn-primary"
                : "btn-outline-primary")
            }
            onClick={() => setPlan("price_1IxuFlSJzInEwQYN8hPRSSLF")}
          >
            Choose Monthly $25/m
          </button>
          ...
          <button
            className={
              "btn " +
              (plan === "price_1IxuFmSJzInEwQYNp3nl04Hv"
                ? "btn-primary"
                : "btn-outline-primary")
            }
            onClick={() => setPlan("price_1IxuFmSJzInEwQYNp3nl04Hv")}
          >
